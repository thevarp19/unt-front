...
export const Navigator = ({ userAnswers, current, toQuestion, answers }) => {
	useEffect(() => {
		...
		const blockElements = document.getElementsByClassName('navigator__block')
		if (navigator && blockElements.length > 0) {
			const blockWidth = blockElements[0].clientWidth
		...
		<div className='navigator__anchor' id='navigator'>
			{userAnswers ? (
				userAnswers.map((e, index) => {
					...
					return (
						<div
							className={`button--hovered navigator__block ${className}`}
							onClick={() => {
								toQuestion(index)
							}}
							key={index}
